...
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        lastUpdateTimeLabel.hidden = YES; //隐藏事件控件
...
- (void)layoutSubviews
{
    ...
    CGFloat statusX = 0;
    CGFloat statusY = 18;
    CGFloat statusHeight = self.height * 0.4;
...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state) {
        ...
        {
            // 设置文字
            if (![[NSUserDefaults standardUserDefaults] boolForKey:@"IS_MJ_HEARD"]) {
//                [playSound playFiles:@"f44"];
                
            }else{
                [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"IS_MJ_HEARD"];
            }
