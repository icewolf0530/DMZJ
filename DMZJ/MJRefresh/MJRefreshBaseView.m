...
- (void)layoutSubviews
{
    ...
    // 1.箭头
    CGFloat arrowX = self.width * 0.5 - 100 + 50;
...
- (void)beginRefreshing
{
    ...
    if (self.window) {
        ...
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"IS_MJ_HEARD"];
    ...
    switch (state) {
		...
		case MJRefreshStateRefreshing:
        {
            ...
            if ([self.beginRefreshingTaget respondsToSelector:self.beginRefreshingAction]) {
                NSMethodSignature * signature = [[self.beginRefreshingTaget class]instanceMethodSignatureForSelector:self.beginRefreshingAction];
                NSInvocation * invocation = [NSInvocation invocationWithMethodSignature:signature];
                invocation.target = self.beginRefreshingTaget;
                invocation.selector = self.beginRefreshingAction;
                [invocation invoke];
            }
